...
/***/ (function(module, exports, __webpack_require__) {
	...
	exports.mapPromise = exports.map = exports.latLonToMarker = exports.markerIcons = undefined;
	...
	exports.updateClearFilters = updateClearFilters;
	exports.setViewClass = setViewClass;

	var _fillDetails = __webpack_require__(4);

	var _photoInfo = __webpack_require__(6);

	var _mapStyles = __webpack_require__(8);

	var _popularPhotos = __webpack_require__(9);
	...
	var URL_FOR_SOCIAL = 'https://oldtoronto.sidewalklabs.com/';

	var markers = [];
	var markerIcons = exports.markerIcons = {
	  images: null, // these are filled in by initializeMap()
	  filtered: null,
	  selected: null,
	  searchPin: null
	};
	var latLonToMarker = exports.latLonToMarker = {};
	var selectedMarker = void 0,
	    selectedIcon = void 0;

	var MIN_YEAR = 1850;
	var MAX_YEAR = 2018;
	var yearRange = [MIN_YEAR, MAX_YEAR];

	var map = exports.map = void 0;
	var mapPromise = exports.mapPromise = $.Deferred();

	var popup = void 0; // initialized in initializeMap()
	...
	function countPhotos(yearToCounts) {
	  if (isFullTimeRange(yearRange)) {
	    ...
	    var _yearRange2 = yearRange,
	        _yearRange3 = _slicedToArray(_yearRange2, 2),
	        first = _yearRange3[0],
	        last = _yearRange3[1];
	...
	function updateYearLabels(opt_yearRange) {
	  var _ref = opt_yearRange || yearRange,
	      _ref2 = _slicedToArray(_ref, 2),
	      firstYear = _ref2[0],
	      lastYear = _ref2[1];

	  var width = $('.time-chart-bg').width();
	  var leftPx = Math.floor((firstYear - MIN_YEAR) / (MAX_YEAR - MIN_YEAR) * width);
	  var rightPx = Math.ceil((lastYear - MIN_YEAR) / (MAX_YEAR - MIN_YEAR) * width);
	  var $fg = $('.time-chart-fg');
	  $fg.css({
	...
	function updateYears(firstYear, lastYear) {
	  ...
	  _.forEach(latLonToMarker, function (marker, lat_lon) {
	    var count = countPhotos(lat_lons[lat_lon], yearRange);
	    if (count) {
	...
	function displayInfoForLatLon(latLon, opt_selectCallback, opt_selectedId) {
	  (0, _photoInfo.loadInfoForLatLon)(latLon).done(function (photoIds) {
	...
	function handleClick(e, opt_latLon) {
	  var latLon = opt_latLon || e.latLng.lat().toFixed(6) + ',' + e.latLng.lng().toFixed(6);
	  var marker = latLonToMarker[latLon];
	  selectMarker(marker, lat_lons[latLon]);

	  (0, _photoInfo.loadInfoForLatLon)(latLon).done(function (photoIds) {
	    var numPhotos = countPhotos(lat_lons[latLon], yearRange);
	    var photoId = void 0;

	    var _yearRange4 = yearRange,
	        _yearRange5 = _slicedToArray(_yearRange4, 2),
	        first = _yearRange5[0],
	        last = _yearRange5[1];
	    ...
	    if (isFullTimeRange(yearRange)) {
	      ...
	      try {
	        for (var _iterator = photoIds[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
	          var id = _step.value;
	    ...
	    if (photoId) {
	      var _info = (0, _photoInfo.infoForPhotoId)(photoId);
	      var width = _info.width,
	          height = _info.height,
	          thumb_url = _info.thumb_url,
	          title = _info.title;

	      var thumbWidth = Math.floor(width / 5);
	      var thumbHeight = Math.floor(height / 5);
	...
	var INIT_CENTER = new google.maps.LatLng(43.6486135, -79.3738487);
	var INIT_ZOOM = 15;
	...
	function initializeMap() {
	  ...
	  var makeMarker = function makeMarker(url, width, height) {
	    return {
	      url: url,
	  ...
	  popup = new _popup.Popup(center, document.createElement('div'));
	...
	function addNewlyVisibleMarkers() {
	  ...
	  for (var latLon in lat_lons) {
	    if (latLon in latLonToMarker) continue;

	    var pos = parseLatLon(latLon);
	...
	function parseLatLon(latLonStr) {
	  var _latLonStr$split = latLonStr.split(","),
	      _latLonStr$split2 = _slicedToArray(_latLonStr$split, 2),
	      lat = _latLonStr$split2[0],
	      lon = _latLonStr$split2[1];
	 ...
	 */
	function createMarker(latLonStr, latLng) {
	  var count = countPhotos(lat_lons[latLonStr], yearRange);
	...
	function showExpanded(key, photoIds, opt_selectedId) {
	  ...
	  if (isFullTimeRange(yearRange)) {
	    ...
	    var _yearRange6 = yearRange,
	        _yearRange7 = _slicedToArray(_yearRange6, 2),
	        first = _yearRange7[0],
	        last = _yearRange7[1];
	  ...
	  var images = $.map(photoIds, function (photoId) {
	    var info = (0, _photoInfo.infoForPhotoId)(photoId);
	...
	function fillPhotoPane(photoId, $pane) {
	  ...
	  var info = (0, _photoInfo.infoForPhotoId)(photoId);
	  (0, _fillDetails.fillDetailsPanel)(photoId, info, $pane);
	  ...
	  var canonicalUrl = URL_FOR_SOCIAL + '/#' + photoId;
	  ...
	  if (typeof twttr !== 'undefined') {
	    twttr.widgets.createShareButton(canonicalUrl, $pane.find('.tweet').get(0), {
	      count: 'none',
	  ...
	  $pane.off("mousewheel").on("mousewheel", function (event) {
	    ...
	    var scrollHeight = $pane.get(0).scrollHeight;
	...
	function fillPopularImagesPanel() {
	  ...
	  var elapsedDays = Math.floor(elapsedMs / 86400 / 1000);
	  var shift = elapsedDays % _popularPhotos.popular_photos.length;
	  ...
	  var makePanel = function makePanel(row) {
	    ...
	    .attr('data-src', row.thumbnail_url).attr('height', row.height);
	...
	function updateClearFilters() {
	  var show = !!_search.locationMarker || !isFullTimeRange(yearRange);
	...
	function setViewClass(view) {
	  var allViews = 'view-grid view-map view-photo';
	...
	function showRandomPhotoWhenIdle() {
	  ...
	  var $clearFiltersEl = $('#clear-filters');
	  ...
	  if (Math.random() > 0.5) {
	    var el = pickRandomElement($('#popular a'));
	    el.click();
	  } else {
	    var latLon = pickRandomElement(Object.keys(lat_lons));
	    (0, _photoInfo.loadInfoForLatLon)(latLon).then(function (photoIds) {
	      var id = pickRandomElement(photoIds);
	...
	$(function () {
	  ...
	  $('#grid-container').on('og-fill', 'li', function (e, div) {
	    ...
	    var isFullScreen = $('.og-expander-inner').css('position') === 'fixed';
	    var photoId = $('#grid-container').expandableGrid('selectedId');
	    if (!isFullScreen) {
	      window.open((0, _photoInfo.libraryUrlForPhotoId)(photoId), '_blank');
	    } else {
	      var imageUrl = (0, _photoInfo.infoForPhotoId)(photoId).image_url;
	      if (imageUrl) {
	  ...
	  $('#grid-container').on('og-select', 'li', function () {
	    var photoId = $(this).data('image-id');
	  ...
	  $('#time-slider').slider({
	    ...
	    stop: function stop(event, ui) {
	      var _ui$values3 = _slicedToArray(ui.values, 2),
	          a = _ui$values3[0],
	          b = _ui$values3[1];
	  ...
	  $('#slideshow-all').on('click', function () {
	    ...
	    var latLonStr = $('#expanded').data('grid-key');
	  ...
	  document.addEventListener('click', function (e) {
	    ...
	    var latLonStr = $link.attr('data-latlon');
	  ...
	  var popupOnClick = function popupOnClick(e) {
	    var $popup = $(e.target).parents('.popup');
	    if ($popup.length === 0) return;

	    e.stopPropagation(); // prevent the click from propagating through to the map.
	    var latLonStr = $popup.attr('data-latlon');
	    var photoId = null; // $popup.attr('data-photoid');
	  ...
	  $(window).on('resize', function () {
	    ...
	    var $pane = $('#expanded div.og-details');
	    if ($pane.length === 0) return;
	    var $img = $pane.find('.inline-image');
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var SITE = '';
	var BY_LOCATION_API = '/api/oldtoronto/by_location';
	...
	function loadInfoForLatLon(latLonStr) {
	  var url = void 0;
	  if (latLonStr === 'pop') {
	  ...
	  return $.getJSON(url).then(function (responseData) {
	    ...
	    photoIds = _.sortBy(photoIds, function (id) {
	      return photoIdToInfo[id].date;
	    });
	    if (latLonStr !== 'pop') {
	...
	// to the empty string.
	function infoForPhotoId(photoId) {
	  return photoIdToInfo[photoId] || { title: '', date: '', library_url: '' };
	}

	// Would it make more sense to incorporate these into infoForPhotoId?
	function descriptionForPhotoId(photoId) {
	  var info = infoForPhotoId(photoId);
	  var desc = info.title;
	  if (desc) desc += ' ';
	  var date = info.date.replace(/n\.d\.?/, 'No Date') || 'No Date';
	  desc += date;
	  return desc;
	}

	function libraryUrlForPhotoId(photoId) {
	  var info = infoForPhotoId(photoId);
	  return info['url'];
	}
...
/***/ (function(module, exports) {
	...
	var popular_photos = exports.popular_photos = [{
	  "id": "452181",
	  "image_url": "https://storage.googleapis.com/sidewalk-old-toronto/images/s1465_fl0377_it0004.jpg",
	  "thumbnail_url": "https://storage.googleapis.com/sidewalk-old-toronto/thumbnails/s1465_fl0377_it0004.jpg",
	  "date": "1970",
	  "title": "City Skyline from Leslie Street Spit, 1970s",
	  "height": 143,
	  "subtitle": ""
	}, {
	  "id": "141995",
	  "image_url": "https://storage.googleapis.com/sidewalk-old-toronto/images/s0372_ss0041_it0387.jpg",
	  "thumbnail_url": "https://storage.googleapis.com/sidewalk-old-toronto/thumbnails/s0372_ss0041_it0387.jpg",
	  "date": "1935",
	  "title": "Cherry Beach, 1935",
	  "height": 145,
	  "subtitle": ""
	}, {
	  "id": "135927",
	  "image_url": "https://storage.googleapis.com/sidewalk-old-toronto/images/f1568_it0311.jpg",
	  "thumbnail_url": "https://storage.googleapis.com/sidewalk-old-toronto/thumbnails/f1568_it0311.jpg",
	  "date": "1903",
	  "title": "Yonge & Temperance, 1903",
	  "height": 137,
	  "subtitle": ""
	}, {
	  "id": "144223",
	  "image_url": "https://storage.googleapis.com/sidewalk-old-toronto/images/s0372_ss0079_it0334.jpg",
	  "thumbnail_url": "https://storage.googleapis.com/sidewalk-old-toronto/thumbnails/s0372_ss0079_it0334.jpg",
	  "date": "1928",
	  "title": "Railyards, 1928",
	  "height": 144,
	  "subtitle": ""
	}, {
	  "id": "51505",
	  "image_url": "https://storage.googleapis.com/sidewalk-old-toronto/images/f1244_it0246.jpg",
	  "thumbnail_url": "https://storage.googleapis.com/sidewalk-old-toronto/thumbnails/f1244_it0246.jpg",
	  "date": "1909",
	  "title": "Crowds waiting to board ferry to Toronto Island, 1909",
	  "height": 147,
	  "subtitle": ""
	}, {
	  "id": "87294",
	  "image_url": "https://storage.googleapis.com/sidewalk-old-toronto/images/f1244_it4535.jpg",
	  "thumbnail_url": "https://storage.googleapis.com/sidewalk-old-toronto/thumbnails/f1244_it4535.jpg",
	  "date": "1915",
	  "title": "Flying boat, 1915",
	  "height": 154,
	  "subtitle": ""
	}, {
	  "id": "43271",
	  "image_url": "https://storage.googleapis.com/sidewalk-old-toronto/images/s0071_it10259.jpg",
	  "thumbnail_url": "https://storage.googleapis.com/sidewalk-old-toronto/thumbnails/s0071_it10259.jpg",
	  "date": "1934",
	  "title": "Congestion caused by double parking, 1934",
	  "height": 151,
	  "subtitle": ""
	}, {
	  "id": "229459",
	  "image_url": "https://storage.googleapis.com/sidewalk-old-toronto/images/f1548_s0393_it0010.jpg",
	  "thumbnail_url": "https://storage.googleapis.com/sidewalk-old-toronto/thumbnails/f1548_s0393_it0010.jpg",
	  "date": "1940",
	  "title": "Ice Boats, Toronto Harbour, pre-1940",
	  "height": 127,
	  "subtitle": ""
	}, {
	  "id": "430146",
	  "image_url": "https://storage.googleapis.com/sidewalk-old-toronto/images/f0124_fl0009_id0105.jpg",
	  "thumbnail_url": "https://storage.googleapis.com/sidewalk-old-toronto/thumbnails/f0124_fl0009_id0105.jpg",
	  "date": "1972",
	  "title": "Crowds at Ontario Place, 1972-1989",
	  "height": 138,
	  "subtitle": ""
	}, {
	  "id": "86674",
	  "image_url": "https://storage.googleapis.com/sidewalk-old-toronto/images/f1244_it2468.jpg",
	  "thumbnail_url": "https://storage.googleapis.com/sidewalk-old-toronto/thumbnails/f1244_it2468.jpg",
	  "date": "1947",
	  "title": "Queen Street West between Simcoe Street and University Avenue, 1926",
	  "height": 152,
	  "subtitle": ""
	}, {
	  "id": "95166",
	  "image_url": "https://storage.googleapis.com/sidewalk-old-toronto/images/s0372_ss0003_it1419.jpg",
	  "thumbnail_url": "https://storage.googleapis.com/sidewalk-old-toronto/thumbnails/s0372_ss0003_it1419.jpg",
	  "date": "1909",
	  "title": "Ontario Barber Shop, 1947",
	  "height": 141,
	  "subtitle": ""
	}, {
	  "id": "53175",
	  "image_url": "https://storage.googleapis.com/sidewalk-old-toronto/images/f1244_it1537.jpg",
	  "thumbnail_url": "https://storage.googleapis.com/sidewalk-old-toronto/thumbnails/f1244_it1537.jpg",
	  "date": "1953",
	  "title": "Scarborough Bluffs point, 1909",
	  "height": 151,
	  "subtitle": ""
	}, {
	  "id": "103794",
	  "image_url": "https://storage.googleapis.com/sidewalk-old-toronto/images/s0372_ss0100_it0180.jpg",
	  "thumbnail_url": "https://storage.googleapis.com/sidewalk-old-toronto/thumbnails/s0372_ss0100_it0180.jpg",
	  "date": "1914",
	  "title": "Queen Street West, 1953",
	  "height": 142,
	  "subtitle": ""
	}, {
	  "id": "87674",
	  "image_url": "https://storage.googleapis.com/sidewalk-old-toronto/images/f1244_it7167.jpg",
	  "thumbnail_url": "https://storage.googleapis.com/sidewalk-old-toronto/thumbnails/f1244_it7167.jpg",
	  "date": "1911",
	  "title": "Spadina Avenue, looking south from Queen Street, 1914",
	  "height": 152,
	  "subtitle": ""
	}, {
	  //   Temporarily removed until we get a better location;
	  //   Ossington and St. Clair do not intersect in the modern street grid.
	  //   "id": "80850",
	  //   "image_url": "https://storage.googleapis.com/sidewalk-old-toronto/images/f1231_it1736.jpg",
	  //   "thumbnail_url": "https://storage.googleapis.com/sidewalk-old-toronto/thumbnails/f1231_it1736.jpg",
	  //   "date": "1900",
	  //   "title": "St. Clair Avenue west from Ossington Avenue, 1911",
	  //   "height": 147,
	  //   "subtitle": ""
	  // }, {
	  "id": "135862",
	  "image_url": "https://storage.googleapis.com/sidewalk-old-toronto/images/f1568_it0248.jpg",
	  "thumbnail_url": "https://storage.googleapis.com/sidewalk-old-toronto/thumbnails/f1568_it0248.jpg",
	  "date": "1900",
	  "title": "St. Clair Avenue West, looking east from Dufferin Street, after 1900",
	  "height": 161,
	  "subtitle": ""
	}, {
	  "id": "96347",
	  "image_url": "https://storage.googleapis.com/sidewalk-old-toronto/images/s0372_ss0058_it0191.jpg",
	  "thumbnail_url": "https://storage.googleapis.com/sidewalk-old-toronto/thumbnails/s0372_ss0058_it0191.jpg",
	  "date": "1913",
	  "title": "University Avenue, South of College Street",
	  "height": 147,
	  "subtitle": ""
	}, {
	  "id": "81004",
	  "image_url": "https://storage.googleapis.com/sidewalk-old-toronto/images/f1231_it1889.jpg",
	  "thumbnail_url": "https://storage.googleapis.com/sidewalk-old-toronto/thumbnails/f1231_it1889.jpg",
	  "date": "1913",
	  "title": "Harbord Street bridge at Bickford Park Ravine",
	  "height": 147,
	  "subtitle": ""
	}, {
	  "id": "229496",
	  "image_url": "https://storage.googleapis.com/sidewalk-old-toronto/images/f1548_s0393_it0253.jpg",
	  "thumbnail_url": "https://storage.googleapis.com/sidewalk-old-toronto/thumbnails/f1548_s0393_it0253.jpg",
	  "date": "1895",
	  "title": "Horticultural Gardens (Allan Gardens) from rosary",
	  "height": 146,
	  "subtitle": ""
	  // Temporarily removed due to user feedback.
	  // }, {
	  //   "id": "50752",
	  //   "image_url": "https://storage.googleapis.com/sidewalk-old-toronto/images/s0376_fl0003_it0012.jpg",
	  //   "thumbnail_url": "https://storage.googleapis.com/sidewalk-old-toronto/thumbnails/s0376_fl0003_it0012.jpg",
	  //   "date": "1890",
	  //   "title": "Shaw Street bridge at Sully Crescent looking north",
	  //   "height": 170,
	  //   "subtitle": ""
	}, {
	  "id": "87650",
	  "image_url": "https://storage.googleapis.com/sidewalk-old-toronto/images/f1244_it7143.jpg",
	  "thumbnail_url": "https://storage.googleapis.com/sidewalk-old-toronto/thumbnails/f1244_it7143.jpg",
	  "date": "1907",
	  "title": "Bay Street, looking north to City Hall",
	  "height": 274,
	  "subtitle": ""
	}, {
	  "id": "306119",
	  "image_url": "https://storage.googleapis.com/sidewalk-old-toronto/images/f1266_it22072.jpg",
	  "thumbnail_url": "https://storage.googleapis.com/sidewalk-old-toronto/thumbnails/f1266_it22072.jpg",
	  "date": "1930",
	  "title": "Victoria Street, George Merritt, 8 years old,103 Bond Street",
	  "height": 269,
	  "subtitle": ""
		}];
...
/***/ (function(module, exports) {
	 ...
	 */
	var Popup = exports.Popup = function (_google$maps$OverlayV) {
	  _inherits(Popup, _google$maps$OverlayV);

	  function Popup(position, content) {
	    _classCallCheck(this, Popup);

	    var _this = _possibleConstructorReturn(this, (Popup.__proto__ || Object.getPrototypeOf(Popup)).call(this, position, content));

	    _this.position = position;
	    _this.contentDiv = content;

	    content.classList.add('popup-bubble-content');

	    var pixelOffset = document.createElement('div');
	    pixelOffset.classList.add('popup-bubble-anchor');
	    pixelOffset.appendChild(_this.contentDiv);
...
/***/ (function(module, exports, __webpack_require__) {
	...
	exports.locationMarker = undefined;
	exports.hideLocationMarker = hideLocationMarker;

	var _viewer = __webpack_require__(5);

	var locationMarker = exports.locationMarker = null; /**
	...
	function setLocation(latLng, title) {
	  ...
	  exports.locationMarker = locationMarker = new google.maps.Marker({
	    position: latLng,
	    map: _viewer.map,
	    title: title,
	    icon: _viewer.markerIcons.searchPin
	  });

	  (0, _viewer.updateClearFilters)();
	}

	function hideLocationMarker() {
	  if (locationMarker) {
	    locationMarker.setMap(null);
	    exports.locationMarker = locationMarker = null;
	  }
	}
...
/***/ (function(module, exports, __webpack_require__) {
	...
	$(function () {
	  ...
	  var fragment = function fragment(state) {
	    return '/' + location.search + '#' + (0, _urlState.stateObjectToHash)(state);
	  };
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var _slicedToArray = function () { function sliceIterator(arr, i) { var _arr = []; var _n = true; var _d = false; var _e = undefined; try { for (var _i = arr[Symbol.iterator](), _s; !(_n = (_s = _i.next()).done); _n = true) { _arr.push(_s.value); if (i && _arr.length === i) break; } } catch (err) { _d = true; _e = err; } finally { try { if (!_n && _i["return"]) _i["return"](); } finally { if (_d) throw _e; } } return _arr; } return function (arr, i) { if (Array.isArray(arr)) { return arr; } else if (Symbol.iterator in Object(arr)) { return sliceIterator(arr, i); } else { throw new TypeError("Invalid attempt to destructure non-iterable instance"); } }; }(); // The URL looks like one of these:
	// /
	// /#photo_id
	// /#g:lat,lon
	// /#photo_id,g:lat,lon
	...
	function transitionToStateObject(targetState) {
	  ...
	  hashToStateObject(stateObjectToHash(targetState), function (state) {
	    ...
	    (0, _viewer.setViewClass)(state.photo_id ? 'view-photo' : state.g ? 'view-grid' : 'view-map');
	    ...
	    if (currentState.g !== state.g) {
	      var latLon = state.g;
	      var count = (0, _viewer.countPhotos)(lat_lons[latLon]);
	      if (state.g === 'pop') {
	        count = (0, _viewer.getPopularPhotoIds)().length;
	      } else {
	        // Highlight the marker, creating it if necessary.
	        var marker = _viewer.latLonToMarker[latLon];
	        var latLng = (0, _viewer.parseLatLon)(latLon);
	        if (!marker) {
	          marker = (0, _viewer.createMarker)(latLon, latLng);
	        }
	        if (marker) {
	      ...
	      (0, _photoInfo.loadInfoForLatLon)(latLon).done(function (photo_ids) {
	...
	function findLatLonForPhoto(photoId, cb) {
	  $.ajax({
	    dataType: "json",
	    url: '/api/layer/oldtoronto/' + photoId,
	    success: function success(featureOrFeatureCollection) {
	      var feature = featureOrFeatureCollection.features ? featureOrFeatureCollection.features[0] : featureOrFeatureCollection;
